...
class SQLExporterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("SQL 数据导出工具")
    ...
    def _build_ui(self):
        # --- 数据库连接区 ---
        conn_frame = ttk.LabelFrame(self.root, text="数据库连接", padding=10)
        conn_frame.pack(fill=tk.X, padx=10, pady=(10, 5))

        row0 = ttk.Frame(conn_frame)
        row0.pack(fill=tk.X, pady=2)
        ttk.Label(row0, text="类型:").pack(side=tk.LEFT)
        self.db_type = ttk.Combobox(row0, values=["MySQL", "PostgreSQL", "SQLite"], state="readonly", width=12)
        ...
        # --- SQL 输入区 ---
        sql_frame = ttk.LabelFrame(self.root, text="SQL 查询", padding=10)
        sql_frame.pack(fill=tk.BOTH, padx=10, pady=5, expand=False)

        self.sql_text = scrolledtext.ScrolledText(sql_frame, height=6, font=("Consolas", 11))
        ...
        self.export_fmt = ttk.Combobox(action_frame, values=["CSV", "Excel (.xlsx)"], state="readonly", width=14)
        ...
        preview_frame.pack(fill=tk.BOTH, padx=10, pady=(5, 10), expand=True)
    ...
    def _on_db_type_change(self, _event=None):
        ...
        if db == "SQLite":
            ...
            self.db_entry.config(state=tk.DISABLED)
            ...
            self.db_entry.config(state=tk.NORMAL)
            self.sqlite_frame.pack_forget()
            self.port_var.set("3306" if db == "MySQL" else "5432")

    def _browse_sqlite(self):
        path = filedialog.askopenfilename(filetypes=[("SQLite DB", "*.db *.sqlite *.sqlite3"), ("All", "*.*")])
        if path:
    ...
    def _run_query(self, sql):
        try:
            ...
            if cursor.description is None:
                self.root.after(0, lambda: messagebox.showinfo("完成", "语句已执行（无返回数据）"))
                self.root.after(0, lambda: self.exec_btn.config(state=tk.NORMAL))
                self.root.after(0, lambda: self.row_count_label.config(text=""))
                return
            ...
            self.root.after(0, lambda: messagebox.showerror("查询错误", str(e)))
            self.root.after(0, lambda: self.exec_btn.config(state=tk.NORMAL))
            self.root.after(0, lambda: self.row_count_label.config(text=""))
    ...
    def _display_results(self):
        ...
        total = len(self.rows)
        shown = min(total, 500)
        self.row_count_label.config(text=f"共 {total} 行（预览 {shown} 行）")
        self.exec_btn.config(state=tk.NORMAL)
